        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center space-x-2 flex-wrap gap-y-1">
            ...
            <span className="text-[11px] font-mono text-zinc-500">
              {scheme.level === 'Central' || scheme.state === 'Central' ? 'Central Scheme' : `State: ${scheme.state}`}
            </span>
          </div>

          <div className={`px-2 py-0.5 rounded-full text-[10px] font-mono font-semibold border ${statusBadgeColor}`}>
            {evaluation.status?.replace('_', ' ') || 'ELIGIBLE'}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 p-3 rounded-lg bg-black/40 border border-white/[0.05] mb-4">
          ...
          <div className="flex items-center space-x-2 pl-2 border-l border-white/[0.06]">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs font-semibold text-zinc-200 truncate" title={scheme.financial_benefit || scheme.benefit || (scheme.benefit_amount ? `₹${scheme.benefit_amount.toLocaleString('en-IN')}` : 'Statutory Grant')}>
